...
export const getUserFavorites = async (userId: string): Promise<string[]> => {
  try {
    const response = await axios.get(`${BASE_URL}/get-favorites`, {
      params: { userId },
    });
...
export const addToFavorites = async (userId: string, placeId: string) => {
  try {
    const response = await axios.post(`${BASE_URL}/add-favorites`, {
      userId,
      placeId,
    });
...
export const removeFromFavorites = async (userId: string, placeId: string) => {
  try {
    const response = await axios.post(`${BASE_URL}/remove-favorite`, {
      userId,
      placeId,
    });
    if (response.status !== 200) {
      throw new Error(
        `Failed to remove from favorites: ${response.status} ${response.statusText}`
      );
    }
  } catch (error) {
...
export const getRecentViews = async (userId: string): Promise<string[]> => {
  try {
    const response = await axios.get(`${BASE_URL}/get-recent-views`, {
      params: { userId },
    });
...
export const updateRecentViews = async (userId: string, placeId: string) => {
  try {
    const response = await axios.post(`${BASE_URL}/update-recent-views`, {
      userId,
      placeId,
    });
    if (response.status !== 200) {
      throw new Error(
        `Failed to update recent searches: ${response.status} ${response.statusText}`
      );
    }
  } catch (error) {
